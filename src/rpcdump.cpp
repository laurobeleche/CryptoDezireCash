...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"cryptodezirecashprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"cryptodezirecashprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Crypto Dezire Cash address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"cryptodezirecashaddress\"\n"
            "\nReveals the private key corresponding to 'cryptodezirecashaddress'.\n"
            "Then the importprivkey can be used with this output\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"cryptodezirecashaddress\"   (string, required) The cryptodezirecash address for the private key\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Crypto Dezire Cash %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"cryptodezirecashaddress\" \"passphrase\"\n"
            "\nEncrypts a private key corresponding to 'cryptodezirecashaddress'.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"cryptodezirecashaddress\"   (string, required) The cryptodezirecash address for the private key (you must hold the key already)\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("bip38encrypt", "\"Cf1QqABAiQ17aorbB2WTcWAr6HF6zQRLmV\" \"mypasphrase\"") +
            HelpExampleRpc("bip38encrypt", "\"Cf1QqABAiQ17aorbB2WTcWAr6HF6zQRLmV\" \"mypasphrase\""));
...
UniValue bip38decrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38decrypt \"cryptodezirecashaddress\" \"passphrase\"\n"
