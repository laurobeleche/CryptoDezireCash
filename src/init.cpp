...
void PrepareShutdown()
{
    ...
    RenameThread("cryptodezirecash-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cryptodezirecashd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the CDZC and zCDZC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), Params().GetDefaultPort(), 35602));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CDZC/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CDZC/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, cryptodezirecash, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CDZC/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Crypto Dezire Cash specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-cdzcstake=<n>", strprintf(_("Enable or disable staking functionality for CDZC inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcdzcstake=<n>", strprintf(_("Enable or disable staking functionality for zCDZC inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:35601"));
    ...
    strUsage += HelpMessageOpt("-backupzcdzc=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCDZC minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcdzcbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCDZC backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizecryptodezirecashamount=<n>     " + strprintf(_("Keep N CDZC anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 35602, 36602));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Crypto Dezire Cash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("cryptodezirecash-loadblk");
...
/** Sanity checks
 *  Ensure that Crypto Dezire Cash is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Crypto Dezire Cash Core is shutting down."));
...
#endif
    // Make sure only a single Crypto Dezire Cash process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Crypto Dezire Cash Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CRYPTODEZIRECASH version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //CRYPTODEZIRECASH: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //CRYPTODEZIRECASH specific: zerocoin and spork DB's
                ...
                // CRYPTODEZIRECASH: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZCDZCMinted();
                        RecalculateZCDZCSpent();
                    }
                    RecalculateCDZCSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // CRYPTODEZIRECASH: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Crypto Dezire Cash Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Crypto Dezire Cash Core to complete") << "\n";
        ...
        zwalletMain = new CzCDZCWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zCDZCWallet
        uiInterface.InitMessage(_("Syncing zCDZC wallet..."));

        bool fEnableZCdzcBackups = GetBoolArg("-backupzcdzc", true);
        pwalletMain->setZCdzcAutoBackups(fEnableZCdzcBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zcdzcTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeCryptoDezireCashAmount = 2;
...
//    }
//
//    nAnonymizeCryptoDezireCashAmount = GetArg("-anonymizecryptodezirecashamount", 0);
//    if (nAnonymizeCryptoDezireCashAmount > 999999) nAnonymizeCrypto Dezire CashAmount = 999999;
//    if (nAnonymizeCryptoDezireCashAmount < 2) nAnonymizeCrypto Dezire CashAmount = 2;
    ...
    LogPrintf("AnonymizeCryptoDezireCashAmount %d\n", nAnonymizeCryptoDezireCashAmount);
    ...
    /* Denominations
       ...
       For example:
       1CDZC+1000 == (.1CDZC+100)*10
       10CDZC+10000 == (1CDZC+1000)*10
    */
