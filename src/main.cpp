...
unsigned int nStakeMinAge = 60 * 60 * 16;
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zCdzc spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zCdzc spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zCdzc spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::REGTEST || Params().NetworkID() == CBaseChainParams::TESTNET) {
        ...
        if (nHeight == 0) {
            ...
            nSubsidy = 8 * COIN;
        } else if (nHeight <= 1300112) { 
            nSubsidy = 7 * COIN;
        } else if (nHeight <= 1400113) { 
            nSubsidy = 6 * COIN;
        } else if (nHeight <= 1500114) { 
            nSubsidy = 5 * COIN;
        } else if (nHeight <= 1600115) { 
            nSubsidy = 4 * COIN;
        } else {
            nSubsidy = 3* COIN;
        }
...
CAmount GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 101) {
        ret = 0; 
            ...
            ret = blockValue * .74;   //2.96
    } else  {
            ret = blockValue * .74;   //2.22
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Crypto Dezire Cash
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zCdzc with serial %s is already in the block %d\n",
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (block.GetBlockTime() > Params().Zerocoin_StartTime()) {
        ...
        // Crypto Dezire CashTor - disable reject block as our blocks are in version 4 since block 1
