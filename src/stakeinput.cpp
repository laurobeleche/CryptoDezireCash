...
bool CCdzcStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CCdzcStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CCdzcStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CCdzcStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CCdzcStake::GetModifier(uint64_t& nStakeModifier)
{
...
CBlockIndex* CCdzcStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;
